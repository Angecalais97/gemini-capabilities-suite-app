  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (url) {
        ...
        setPrompt(''); // Optional: clear prompt
      } else {
    ...
    <div className="flex flex-col h-full bg-slate-900 p-6 overflow-y-auto">
      <div className="max-w-4xl mx-auto w-full space-y-8">
        <div className="text-center space-y-2">
          ...
          <p className="text-slate-400">Powered by the fast and efficient Gemini 2.5 Flash Image model.</p>
        </div>
        ...
        <form onSubmit={handleGenerate} className="flex flex-col sm:flex-row gap-3">
          ...
          >
            {loading ? <LoaderIcon className="w-5 h-5" /> : <SparklesIcon className="w-5 h-5" />}
            Generate
          </button>
        </form>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          {generatedImages.map((img, idx) => (
            <div key={idx} className="group relative aspect-square bg-slate-800 rounded-2xl overflow-hidden border border-slate-700">
        ...
        {generatedImages.length === 0 && !loading && (
          <div className="text-center py-20 border border-dashed border-slate-800 rounded-2xl">
            ...
            <p className="text-slate-500">Your imagination is the limit. Try a prompt above!</p>
          </div>
        )}
      </div>
    </div>
