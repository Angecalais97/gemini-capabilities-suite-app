  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        // Strip data:image/jpeg;base64, prefix for the API if needed, 
        // but inlineData.data usually expects pure base64.
        // We will strip it in the service call or here.
        // For preview, we keep the full string.
    ...
    <div className="flex flex-col h-full bg-slate-900 p-6 overflow-y-auto">
      <div className="max-w-3xl mx-auto w-full space-y-8">
        <div className="text-center space-y-2">
          ...
          <p className="text-slate-400">Upload an image and ask Gemini to describe it, extract text, or answer questions about it.</p>
        </div>
        ...
        <div 
          ...
          className={`border-2 border-dashed rounded-2xl p-8 flex flex-col items-center justify-center cursor-pointer transition-colors ${
            previewUrl ? 'border-blue-500 bg-slate-800/50' : 'border-slate-700 hover:border-slate-600 hover:bg-slate-800'
          }`}
        >
          {previewUrl ? (
        ...
        <div className="space-y-4">
          <textarea
            ...
            placeholder="What would you like to know about this image? (e.g., 'Describe the scene', 'Read the text')"
        ...
        {result && (
          <div className="bg-slate-800 rounded-xl p-6 border border-slate-700">
            <h3 className="text-sm font-semibold text-slate-400 mb-2 uppercase tracking-wider">Analysis Result</h3>
            <p className="text-slate-100 whitespace-pre-wrap leading-relaxed">{result}</p>
          </div>
        )}
      </div>
    </div>
